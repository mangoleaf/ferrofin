...
//!
//! Boots the actual composition root ([`hermit_server::state::build_app_state`])
//! over a fresh temp database seeded with a default administrator (via the real
//! [`hermit_server::seed::seed_default_admin`]) and a couple of scanned movie
...
//! [`hermit_api::create_router`] (no network sockets):
//!
...
//! in its body, and Hermit's `SessionManager` now returns it too (via
...
use hermit_server::config::Config;
use hermit_server::state::{WiredApp, build_app_state};
use hermit_traits::devices::DeviceQuery;
...
fn test_config(root: &std::path::Path) -> Config {
    Config {
        ...
        server_name: "hermit-first-light".to_owned(),
...
fn movie_item(id: Uuid, name: &str, path: &str) -> BaseItemEntity {
    let type_name =
        hermit_core::item_type_lookup::stored_type_name(hermit_model::data::BaseItemKind::Movie)
...
async fn boot() -> Harness {
    ...
    let outcome = hermit_server::seed::seed_default_admin(wired.state.users.as_ref(), &config)
        .await
        .expect("seed admin");
    assert!(
        matches!(
            outcome,
            hermit_server::seed::SeedOutcome::SeededWithConfiguredPassword { .. }
        ),
...
async fn first_light_client_flow() {
    ...
    let router = hermit_api::create_router(harness.wired.state.clone());
