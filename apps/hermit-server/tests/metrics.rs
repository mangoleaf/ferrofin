...
//!
//! `apps/hermit-server` is coverage-exempt, but this must pass: it exercises the
...
use hermit_metrics::RouteLabels;
use hermit_server::config::Config;
use hermit_server::metrics_wiring;
...
fn test_config(root: &std::path::Path) -> Config {
    Config {
        ...
        server_name: "hermit-metrics-test".to_owned(),
...
async fn metrics_disabled_by_default_and_route_absent() {
    ...
    let (status, _) = get(hermit_api::create_router(wired.state), "/metrics").await;
...
async fn metrics_enabled_serves_exposition() {
    ...
    let handle =
        hermit_metrics::init(RouteLabels::default(), tokio::runtime::Handle::current()).unwrap();
    let router = metrics_wiring::mount(hermit_api::create_router(wired.state.clone()), &handle);
    ...
    for name in [
        ...
        "hermit_db_pool_connections",   // sampler-fed gauge
    ] {
        assert!(
            body.contains(name),
