//! Hermit server binary entry point.
//!
//! Parses CLI flags, resolves the bootstrap [`Config`](hermit_server::config::Config),
//! and hands off to [`hermit_server::run`], which owns the whole boot-and-serve
...
use hermit_server::config::{Cli, Config};

#[tokio::main]
async fn main() -> anyhow::Result<()> {
    let cli = Cli::parse();
    let config = Config::load(cli)?;
    hermit_server::run(config).await
}
