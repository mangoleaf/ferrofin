...
endpoint_names() {
  node -e '
    ...
    s+="\nprocess.stdout.write(ENDPOINTS.map(e=>e.name).join(\"\n\"));";
    new Function("__ENV", s)({});'
}
# usage_usec (total CPU µs charged to the container's cgroup).
cpu_usec() { docker compose exec -T "$1" cat /sys/fs/cgroup/cpu.stat 2>/dev/null | awk '/^usage_usec/{print $2}'; }
...
profile() {   # $1=service $2=port $3=target
  ...
  local i0 i1 idle_rate; i0=$(cpu_usec "$svc"); sleep "$IDLE_SECS"; i1=$(cpu_usec "$svc")
  idle_rate=$(awk -v a="$i0" -v b="$i1" -v s="$IDLE_SECS" 'BEGIN{printf "%.0f",(b-a)/s}')   # µs/sec idle
  echo "   idle CPU: ${idle_rate} µs/s"

  local total_s; total_s=$(awk -v w="${PHASE_WARMUP%s}" -v d="${PHASE_DUR%s}" 'BEGIN{print w+d}')
  while IFS= read -r name; do
    local c0 c1 dcpu; c0=$(cpu_usec "$svc")
    k6 run -e ENDPOINT="$name" -e TARGET="$target" -e BASE_URL="$base" phase-a.js >/dev/null 2>&1 || true
    c1=$(cpu_usec "$svc")
    # CPU µs consumed by the endpoint's requests = total delta minus idle burn over the window.
    dcpu=$(awk -v a="$c0" -v b="$c1" -v idle="$idle_rate" -v s="$total_s" 'BEGIN{d=(b-a)-idle*s; print (d<0?0:d)}')
    local f="results/raw/phaseA-${target}-${name}.json"
    if [ -f "$f" ]; then
      node -e "const d=require('./$f'); d.cpu_us=$dcpu; d.cpu_us_per_req=d.reqs?($dcpu/d.reqs):null; require('fs').writeFileSync('./$f',JSON.stringify(d));"
      printf '   %-24s p50=%s cpu/req=%.1fµs\n' "$name" "$(node -pe "require('./$f').p50")" "$(node -pe "require('./$f').cpu_us_per_req||0")"
    fi
  done < <(endpoint_names)
