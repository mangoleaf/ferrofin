...
> **Living document.** Mandatory rules for metric collection in the `hermit-*`
> workspace. These govern every instrument added to the codebase, now and later.
> The implementation lives in `crates/hermit-metrics`; this file is the
> design contract the implementation must satisfy.

# Metrics conventions

Hermit exposes Prometheus text-exposition metrics on `GET /metrics`, gated by the
...
an encode-time detail inside `hermit-metrics`.
...
- A metric whose concept exists in Jellyfin's output keeps Jellyfin's **exact
  ...
  family. Existing community Jellyfin Grafana dashboards must work against Hermit
...
- **Never fake a .NET metric.** `dotnet_gc_*`, `dotnet_jit_*`, `dotnet_exceptions_*`,
  ...
  analogue of `dotnet_threadpool_*` is `hermit_tokio_*` from
  `tokio::runtime::RuntimeMetrics`.
- The parity name-set test (metrics analogue of `contract_superset.rs`) asserts
  every portable Jellyfin metric name renders in Hermit's exposition. Keep it green.
...
- The `prometheus` crate is a **direct dep of `hermit-metrics` only**,
...
- There is exactly **one recording pipeline**: OTel global meter → prometheus
  ...
  workspace learned this the hard way; Hermit avoids the split by construction.
- Hermit is a standalone workspace: metrics are built on the `prometheus`
  crate directly, never on external in-house telemetry crates.

## 3. Naming

- Parity metrics: Jellyfin's names verbatim (rule 1). Everything Hermit-specific:
  `hermit_` prefix, snake_case, base units in the name (`_seconds`, `_bytes`,
...
- The toggle is resolved once at startup in the composition root: the bootstrap
  override `HERMIT_ENABLE_METRICS` (env) / `enable_metrics` (`config.toml`) wins
...
- Instruments live in `crates/hermit-metrics` (HTTP + process families) or the
  composition root `apps/hermit-server` (sampler-fed gauges). That is the default
  and covers everything shipped so far.
- `hermit-api` handlers and library crates must **not** depend on
  `hermit-metrics` — same DI principle as `hermit-traits` (handlers see traits,
  not impls). A subsystem that later genuinely needs deep-internal recording uses
  `opentelemetry::global::meter("hermit")` directly; that dep is `opentelemetry`
  only, and the noop-when-disabled guarantee (rule 6) still holds.
- Do not confuse this system with the `PlaybackMetrics` trait
  (`hermit-traits/src/metrics.rs`) — that is the DB-backed playback-decision log,
...
- New metric ⇒ a row in the table in `contrib/metrics/README.md`, marked
  **parity** or **hermit-specific**; consider whether the Grafana dashboard
...
- `/metrics` is served on the **app port**, like Jellyfin — not a dedicated
  ...
  Hermit is a LAN media server whose exposure model is Jellyfin's, and matching
  Jellyfin's behavior is the product requirement. If Hermit ever grows a
...
- Scrape cadence assumption is 15 s (`contrib/metrics/prometheus.yml`); the
  ...
  `HERMIT_METRICS_SAMPLE_INTERVAL` (env) / `metrics_sample_interval` (`config.toml`)
...
1. Does Jellyfin expose this concept? → use its exact name/labels/buckets (check
   the fixture). Otherwise → `hermit_*` naming rules.
