...
suite_gen_fixtures() {
  ...
  elif [ "${FIXTURE_LIVETV:-0}" -gt 0 ] && { [ ! -f fixtures/media/livetv/channels.m3u ] \
        || [ ! -f fixtures/media/livetv/guide.xml ] || [ ! -f fixtures/media/livetv/loop.ts ]; }; then
    # The FILES, not the dir: a tree regenerated by a run without the livetv
    # flag leaves the dir with only some of them, and the M3U importing while
    # the guide is missing shows up as "channels/programmes never appeared".
    echo ">> regenerating synthetic fixtures (live tv fixture incomplete)"; ./gen-fixtures.sh
  fi
}
