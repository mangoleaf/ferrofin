...
# Returns (ratio, is_tie); is_tie marks exactly the floor-suppressed case so
# ratio_excluded_ties derives from this one function (round 3).
assert merge.floored_speedup(100, 300, FLOOR) == (3.0, False)
assert merge.floored_speedup(0.1, 0.3, FLOOR) == (None, True)   # the "391×" class
assert merge.floored_speedup(10, 13, FLOOR) == (1.3, False)     # exactly-floor: real
assert merge.floored_speedup(None, 300, FLOOR) == (None, False)
assert merge.floored_speedup(100, None, FLOOR) == (None, False)
assert merge.floored_speedup(0, 1, FLOOR) == (None, True)       # zero + sub-floor: tie
assert merge.floored_speedup(0, 300, FLOOR) == (None, False)    # zero + clear: no ratio
