...
description: >-
  Check the third-party Jellyfin plugins ported into ferrofin-extensions against
...
Keep Ferrofin's compiled-in plugin ports faithful to their upstream repos. The
...
Ferrofin code is faithful to). Run from the repo root.
...
- **Not applicable** — .NET packaging, DI plumbing, Jellyfin-internal
  representation Ferrofin doesn't model. Check the manifest's **accepted
  divergences** for that plugin first — never "fix" a listed divergence.
- **Upstream bug** — if upstream is buggy and Ferrofin is correct, keep Ferrofin
...
The manifest's per-plugin notes list the Ferrofin files. Port faithfully from the
...
handlers only touch `ferrofin-traits`, object-safe traits, PascalCase DTOs.

If dashboard assets changed (`Configuration/*.html|js|css`), bump the plugin's
`*_REV` const in `crates/ferrofin-extensions/build.rs` to the new rev, rebuild
with `FERROFIN_REFRESH_PLUGIN_ASSETS=1 cargo build -p ferrofin-extensions`, and
commit the refreshed `crates/ferrofin-extensions/assets/`.
...
If `ferrofin-core`, `ferrofin-db`, or `ferrofin-api` were touched:
