...
description: >-
  Keep charts/ferrofin/values.example.yaml (the shareable, de-identified worked
  example) in sync with the chart schema (charts/ferrofin/values.yaml) and with
  Ferrofin's real behavior. Use when the chart's values.yaml or templates change,
  when a new FERROFIN_* setting lands, or when asked to "update the example
...
`charts/ferrofin/values.example.yaml` is a **worked, de-identified** example we
...
1. **The chart schema** — `charts/ferrofin/values.yaml` (defaults + `# --` docs) and
   `charts/ferrofin/templates/` (what keys are actually consumed). The example may
   only use keys the chart understands.
2. **Ferrofin's real behavior** — the app code. Env keys are resolved in
   `apps/ferrofin-server/src/config.rs` (grep `FERROFIN_`); the on-disk layout is
   `crates/ferrofin-core/src/app_paths.rs` + `crates/ferrofin-traits/src/system.rs`.
   Claims in comments (e.g. "Ferrofin exposes /metrics") must match the code.

The **optional** working reference is the real homelab config at
`~/dev/lab/services/ferrofin/values.yaml` — mine it for realistic patterns, but the
example must be a **de-identified generalization**, never a copy.

## Procedure

1. **Diff schema vs example.** List top-level keys in `charts/ferrofin/values.yaml`
   ...
   persistence, volumes, volumeMounts, ferrofin.env/config/envFrom/args, metrics,
...
2. **Check env keys.** `grep -roE 'FERROFIN_[A-Z_]+' apps/ferrofin-server/src/config.rs`
   and compare to the `ferrofin.config` block. New user-facing settings (e.g. a new
   `FERROFIN_*_DIR`, provider key) should appear as a commented example line.
...
4. **Fix stale claims.** Cross-check comment assertions against code. Known trap:
   the chart's own `values.yaml` once said "Ferrofin does not expose /metrics yet"
...
5. **De-identification gate — MUST pass before finishing.** Run:
   ```
   grep -niE '<private-hostname-or-org>|nvme0|/mnt/|k3s|10\.|192\.168|[a-z0-9.-]+\.svc\.cluster\.local' charts/ferrofin/values.example.yaml
   ```
...
6. **Validate YAML.**
   ```
   python3 -c "import yaml; list(yaml.safe_load_all(open('charts/ferrofin/values.example.yaml')))"
   ```
...
7. **Final review — prove the example is compatible with the chart itself.**
   ...
   `ferrofin.config` renders fine and is dropped. The only real proof is rendering
   the chart with the example and confirming the values land in the output.

   a. **Render must succeed** (exit 0), and lint clean:
      ```
      helm template ferrofin charts/ferrofin -f charts/ferrofin/values.example.yaml > /tmp/rendered.yaml
      helm lint charts/ferrofin -f charts/ferrofin/values.example.yaml
      ```
   b. **Round-trip every distinctive value** — each must appear in the rendered
      ...
      for pat in '/data/cache' 'nvidia.com/gpu' '<registry-pull-secret>' \
                 'FERROFIN_ENABLE_METRICS' '/media/TV' 'privileged: true' \
                 '<storage-node>' '<ferrofin-cache-pvc>'; do
