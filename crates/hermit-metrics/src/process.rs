//! The `process_*` family (prometheus-net parity) plus the `hermit_tokio_*`
...
/// `hermit_tokio_*` — the honest analogue of `dotnet_threadpool_*`.
fn register_tokio(meter: &Meter, runtime: Handle) {
    let workers = runtime.clone();
    meter
        .u64_observable_gauge("hermit_tokio_workers")
        ...
        .build();
    meter
        .u64_observable_gauge("hermit_tokio_alive_tasks")
