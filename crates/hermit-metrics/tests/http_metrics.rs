...
//!
//! These live in their own binary because [`hermit_metrics::init`] sets the
...
async fn tracks_requests_and_satisfies_parity_name_set() {
    let handle = hermit_metrics::init(stub_labels(), tokio::runtime::Handle::current())
        .expect("metrics init");
    let app = app();
    ...
    for name in PORTABLE_JELLYFIN_METRICS {
        assert!(
            out.contains(name),
            "portable Jellyfin metric `{name}` is absent from Hermit's exposition:\n{out}"
        );
    }
}

/// The Jellyfin (prometheus-net) metric names whose concept exists in Rust and
/// that Hermit therefore ports 1:1. Kept in sync with
