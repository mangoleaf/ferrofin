...
//! shell plus a built JS/CSS app). Ferrofin compiles extensions in, so those
//! assets must be baked into `ferrofin-extensions` — and they must be the *real*
...
//! 2. Otherwise — or when `FERROFIN_REFRESH_PLUGIN_ASSETS` is set — clone the
...
//! `FERROFIN_REFRESH_PLUGIN_ASSETS=1`, then commit the refreshed `assets/`.
...
fn main() {
    println!("cargo:rerun-if-env-changed=FERROFIN_REFRESH_PLUGIN_ASSETS");
    let manifest = PathBuf::from(env::var("CARGO_MANIFEST_DIR").expect("CARGO_MANIFEST_DIR"));
    let out_dir = PathBuf::from(env::var("OUT_DIR").expect("OUT_DIR"));
    let refresh = env::var_os("FERROFIN_REFRESH_PLUGIN_ASSETS").is_some();
