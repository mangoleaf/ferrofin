...
use hermit_model::data::{BaseItemKind, CollectionType, MediaType, UnratedItem};
use hermit_model::dto::SortOrder;
use hermit_model::entities::{ExtraType, ImageType, SeriesStatus, TrailerType, VideoType};
use hermit_model::entities_media::ParentalRatingScore;
use hermit_model::live_tv::ItemSortBy;
use hermit_model::querying::ItemFilter;
...
/// `hermit-model`** (see the port report's belongs-in-model flags). Declared
/// here so [`InternalItemsQuery::source_types`] can be typed faithfully; move to
/// `hermit-model` if it becomes a wire type.
