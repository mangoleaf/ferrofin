...
//! Skips itself unless `HERMIT_FFMPEG_TESTS` is set *and* `ffmpeg` is on
//! `PATH`, so ffmpeg-less CI stays green.
//!
//! Run with:
//! `HERMIT_FFMPEG_TESTS=1 cargo test -p hermit-mediaencoding --test trickplay_frames_ffmpeg`

use std::path::Path;
use std::sync::Arc;

use hermit_mediaencoding::{TokioTranscoder, TrickplayFrameExtractorImpl};
