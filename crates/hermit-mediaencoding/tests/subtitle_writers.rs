...
use hermit_mediaencoding::subtitles::model::{
    Paragraph, Subtitle, TICKS_PER_MILLISECOND, TimeCode,
};
use hermit_mediaencoding::subtitles::{SubtitleEditParser, SubtitleParser};
use hermit_mediaencoding::subtitles::{json_writer, srt, ssa, vtt};
...
fn parser_to_track_info_projects_cues() {
    ...
    let data = "1\n00:00:01,000 --> 00:00:02,000\nhi\n".as_bytes();
    let parsed = p.parse(data, "srt").unwrap();
    let info = hermit_mediaencoding::subtitles::parser::to_track_info(&parsed);
