//! Error type for the `hermit-mediaencoding` crate.
//!
//! The media-encoding traits in `hermit-traits` (`MediaEncoder`,
//! `SubtitleEncoder`, `AttachmentExtractor`, `TrickplayFrameExtractor`, …) return
//! [`ServiceError`](hermit_traits::error::ServiceError), so the public methods
...
//! [`ServiceError::BackendSource`](hermit_traits::error::ServiceError::BackendSource),
...
/// A backend/infrastructure failure raised inside `hermit-mediaencoding` while
