...
//!   `ferrofin-db` display-preferences rows. Identity arguments become
...
pub trait ServerConfigurationManager: Send + Sync {
    ...
    /// Ferrofin persists it alongside the main configuration. A never-configured
    ...
    ///
    /// Jellyfin stores this as a named configuration in a pluggable store;
    /// Ferrofin persists it alongside the main configuration. The default
    ...
    /// `FerrofinServerConfigurationManager` overrides it to read the persisted
