//! [`FerrofinActivityManager`] — the concrete [`ActivityManager`] over `ferrofin-db`.
//!
...
#[derive(Clone)]
pub struct FerrofinActivityManager {
    db: Database,
}

impl std::fmt::Debug for FerrofinActivityManager {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("FerrofinActivityManager")
            .finish_non_exhaustive()
    }
}

impl FerrofinActivityManager {
...
#[async_trait]
impl ActivityManager for FerrofinActivityManager {
...
mod tests {
    ...
    async fn returns_entries_ordered_newest_first() {
        ...
        let mgr = FerrofinActivityManager::new(db);
        let result = mgr
