//! [`FerrofinApiKeyManager`] — the concrete [`ApiKeyManager`] over `ferrofin-db`.
//!
...
use ferrofin_db::entities::security::ApiKeyEntity;
...
#[derive(Clone)]
pub struct FerrofinApiKeyManager {
    db: Database,
}

impl std::fmt::Debug for FerrofinApiKeyManager {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("FerrofinApiKeyManager")
            .finish_non_exhaustive()
    }
}

impl FerrofinApiKeyManager {
...
#[async_trait]
impl ApiKeyManager for FerrofinApiKeyManager {
...
mod tests {
    ...
    async fn create_then_list_reflects_new_key() {
        ...
        let mgr = FerrofinApiKeyManager::new(db);
