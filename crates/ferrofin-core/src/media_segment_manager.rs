//! [`FerrofinMediaSegmentManager`] — a **minimal** [`MediaSegmentManager`] over the
//! `MediaSegments` table.
//!
...
//! rows (`MediaSegments`) directly over `ferrofin-db` (there is no separate
//! repository trait for this leaf table), resolving an item's kind through the
//! injected [`LibraryManager`](ferrofin_traits::library::LibraryManager) to answer
//! [`MediaSegmentManager::is_type_supported`]. Rows cross the boundary as the
//! [`MediaSegmentDto`] wire type, reusing the `ferrofin-db`
//! [`MediaSegmentEntity`](ferrofin_db::entities::playback::MediaSegmentEntity) →
//! DTO conversion.
//!
...
use ferrofin_db::entities::playback::MediaSegmentEntity;
