//! [`FerrofinChapterManager`] — a **minimal** [`ChapterManager`] over the
...
//! [`ChapterRepository`](ferrofin_traits::persistence::ChapterRepository) and
//! resolves an item's kind through the injected
//! [`LibraryManager`](ferrofin_traits::library::LibraryManager) to answer
...
use ferrofin_db::entities::base_items::ChapterEntity;
...
impl std::fmt::Debug for FerrofinChapterManager {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("FerrofinChapterManager")
            .finish_non_exhaustive()
    }
}

impl FerrofinChapterManager {
...
#[async_trait]
impl ChapterManager for FerrofinChapterManager {
...
mod tests {
    ...
    use crate::chapter_repository::FerrofinChapterRepository;
    use crate::test_support::{library_manager_over, seed_item, test_db};

    use super::FerrofinChapterManager;
    ...
    async fn save_get_and_delete_round_trip() {
        ...
        let repo = Arc::new(FerrofinChapterRepository::new(db.clone()));
        let library = library_manager_over(db.clone());
        let mgr = FerrofinChapterManager::new(repo, library);
    ...
    async fn supports_only_video_kinds() {
        ...
        let repo = Arc::new(FerrofinChapterRepository::new(db.clone()));
        let mgr = FerrofinChapterManager::new(repo, library_manager_over(db.clone()));
