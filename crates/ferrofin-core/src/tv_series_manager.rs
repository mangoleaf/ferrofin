...
//! [`NextUpService`](ferrofin_traits::persistence::NextUpService) and then
...
mod tests {
    ...
    impl LibraryManager for FakeLibraryManager {
        ...
        async fn query_items(
            ...
            Ok(ferrofin_model::querying::QueryResult::new(
                None,
                None,
                Vec::new(),
            ))
    ...
    async fn seeded_user(db: &ferrofin_db::Database, id: Uuid) -> UserEntity {
        seed_user(db, id).await
    }

    /// Seeds a real `Episode` `BaseItems` row and reads it back as an entity.
    async fn seeded_episode(db: &ferrofin_db::Database, id: Uuid) -> BaseItemEntity {
    ...
    async fn seeded_folder(db: &ferrofin_db::Database, id: Uuid) -> BaseItemEntity {
        crate::test_support::seed_item(db, id, ferrofin_model::data::BaseItemKind::Folder).await;
        sqlx::query_as::<_, BaseItemEntity>(r#"SELECT * FROM "BaseItems" WHERE "Id" = ?1"#)
            .bind(guid_to_db(id))
            .fetch_one(db.pool())
            .await
