...
//! do not live in `hermit-model` — they mirror Jellyfin's
...
//!
//! Two closely related enums — [`hermit_model::entities::MetadataField`] and
//! [`hermit_model::entities::TrailerType`] — already exist in `hermit-model`
...
use hermit_model::entities::{MetadataField, TrailerType};
...
db_enum! {
    /// The kind of a linked child (`LinkedChildren.ChildType`).
...
/// `i32` mapping helpers for [`hermit_model::entities::MetadataField`].
///
/// `MetadataField` is reused from `hermit-model` (identical discriminants);
...
/// `i32` mapping helpers for [`hermit_model::entities::TrailerType`].
///
/// `TrailerType` is reused from `hermit-model` (identical discriminants);
