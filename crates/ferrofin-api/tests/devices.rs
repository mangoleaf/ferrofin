//! Devices handler tests: device listing, info, options, and deletion.
//!
...
//! wiring every manager); the ApiKeys/ClientLog/Config stubs the harness defines
...
use ferrofin_api::test_support::{
    FakeAppHost, FakeClientEventLogger, FakeCollections, FakeConfig, FakeDto, FakeLibrary,
    FakeLyrics, FakeMediaSegments, FakeMediaSources, FakeMusic, FakePlaylists, FakeQuickConnect,
    FakeSearch, FakeSessions, FakeSimilarItems, FakeSubtitles, FakeSystem, FakeTrickplay,
...
use ferrofin_model::dto::DeviceInfoDto;
...
use ferrofin_traits::security::ApiKeyManager;
...
impl DeviceManager for StubDevices {
    ...
    ) -> Result<ferrofin_model::session::ClientCapabilities, ServiceError> {
        unimplemented!()
    }
    ...
    async fn get_device_infos(
        &self,
        _query: &DeviceQuery,
    ) -> Result<QueryResult<ferrofin_model::devices::DeviceInfo>, ServiceError> {
        unimplemented!()
    }
    ...
    async fn to_client_capabilities_dto(
        &self,
        _capabilities: &ferrofin_model::session::ClientCapabilities,
    ) -> Result<ferrofin_model::dto::ClientCapabilitiesDto, ServiceError> {
        unimplemented!()
    }
}
