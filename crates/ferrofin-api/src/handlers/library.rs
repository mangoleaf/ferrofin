...
const REFRESH_LIBRARY_TASK: &str = "RefreshLibrary";
...
async fn refresh_library(
    ...
    if let Err(err) = state.tasks.cancel_task(REFRESH_LIBRARY_TASK).await {
        tracing::error!(%err, "Error refreshing library");
    }
    if let Err(err) = state.tasks.start_task(REFRESH_LIBRARY_TASK).await {
        tracing::error!(%err, "Error refreshing library");
    }
