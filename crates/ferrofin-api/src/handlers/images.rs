...
//! [`UserManager::get_profile_image`], both backed by real `ferrofin-db` reads.
//!
//! ## Port scope — processed serving
//!
//! Every image request runs through the wired
//! [`ImageProcessor`](ferrofin_traits::drawing::ImageProcessor) (the real
...
use ferrofin_traits::options::{ImageProcessingOptions, ItemImageInfo};
...
/// size/format/blur fields drive the [`ImageProcessor`](ferrofin_traits::drawing::ImageProcessor)
...
async fn project_image_infos(
    images: &[ItemImageInfo],
    processor: Option<&dyn ferrofin_traits::drawing::ImageProcessor>,
    item_id: Uuid,
) -> Vec<ImageInfo> {
...
///
/// When an [`ImageProcessor`](ferrofin_traits::drawing::ImageProcessor) is present
...
mod tests {
    ...
    /// concrete `ferrofin-drawing` processor `DtoService` uses for `ImageTags`.
    struct TagProcessor;

    #[async_trait::async_trait]
    impl ferrofin_traits::drawing::ImageProcessor for TagProcessor {
        ...
        async fn get_image_dimensions(
            &self,
            _path: &str,
        ) -> Result<ferrofin_model::drawing::ImageDimensions, ferrofin_traits::error::ServiceError>
        {
            Err(ferrofin_traits::error::ServiceError::NotFound(
                "unused".into(),
            ))
        }
        ...
        ) -> Result<ferrofin_traits::drawing::ProcessedImage, ferrofin_traits::error::ServiceError>
        {
            Err(ferrofin_traits::error::ServiceError::NotFound(
                "unused".into(),
            ))
        ...
        async fn get_image_blur_hash_sized(
            &self,
            _path: &str,
            _image_dimensions: ferrofin_model::drawing::ImageDimensions,
        ) -> Result<String, ferrofin_traits::error::ServiceError> {
            Err(ferrofin_traits::error::ServiceError::NotFound(
                "unused".into(),
            ))
    ...
    async fn image_info_size_is_file_length_for_local_files() {
        ...
        let dir = std::env::temp_dir().join(format!("ferrofin-img-{}", std::process::id()));
