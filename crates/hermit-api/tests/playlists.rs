...
impl PlaylistManager for RecordingPlaylists {
    ...
    async fn update_playlist(
        &self,
        _request: &hermit_model::playlists::PlaylistUpdateRequest,
    ) -> Result<(), ServiceError> {
        Ok(())
    }
    ...
    async fn add_user_to_shares(
        &self,
        _request: &hermit_model::playlists::PlaylistUserUpdateRequest,
    ) -> Result<(), ServiceError> {
        Ok(())
    }
    async fn remove_user_from_shares(
        ...
        _playlist_id: Uuid,
        _user_id: Uuid,
        _share: &hermit_model::entities_media::PlaylistUserPermissions,
    ) -> Result<(), ServiceError> {
        Ok(())
    }
...
fn state(playlists: Arc<RecordingPlaylists>, collections: Arc<RecordingCollections>) -> AppState {
    AppState::new(
        Arc::new(hermit_api::test_support::FakeLibrary),
        Arc::new(OkUsers),
...
async fn get_playlist_users_returns_empty_shares() {
    ...
    .await;
    assert_eq!(status, StatusCode::OK);
    let shares: Vec<hermit_model::entities_media::PlaylistUserPermissions> =
