...
use hermit_model::dto::MetadataEditorInfo;
...
fn entity_to_dto(item: &BaseItemEntity) -> hermit_model::dto::BaseItemDto {
    hermit_model::dto::BaseItemDto {
        id: Uuid::parse_str(&item.id).unwrap_or_else(|_| Uuid::nil()),
        name: item.name.clone(),
        ..hermit_model::dto::BaseItemDto::default()
    }
}
...
impl DtoService for OkDto {
    ...
    ) -> Result<hermit_model::dto::BaseItemDto, ServiceError> {
    ...
    ) -> Result<Vec<hermit_model::dto::BaseItemDto>, ServiceError> {
