...
use hermit_api::test_support::minimal_base_item;
...
use hermit_traits::options::{AuthorizationInfo, InternalItemsQuery};
...
impl AttachmentExtractor for FakeAttachments {
    ...
    async fn extract_all_attachments(
        &self,
        _input_file: &str,
        _media_source: &hermit_model::dto::MediaSourceInfo,
    ) -> Result<(), ServiceError> {
        Ok(())
    }
}
...
impl TempFile {
    fn new() -> Self {
        ...
        p.push(format!("hermit-hls-{}.ts", Uuid::new_v4()));
...
#[async_trait]
impl hermit_traits::library::MediaSourceManager for NoStaticSources {
    ...
    ) -> Result<Vec<hermit_model::dto::MediaSourceInfo>, ServiceError> {
        Ok(Vec::new())
    }
...
fn router_no_static(hls: Arc<dyn HlsStreamManager>) -> axum::Router {
    use hermit_api::test_support as t;
