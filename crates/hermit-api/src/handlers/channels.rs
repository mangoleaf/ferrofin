...
//! plugins. Hermit has no built-in channel providers, so — exactly like a stock
...
use hermit_model::channels::ChannelFeatures;
...
#[utoipa::path(
    ...
    // `utoipa::ToSchema` (a `hermit-model` DTO defect) — see `items::get_items`.
