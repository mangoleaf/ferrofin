...
//! always included (they are the restorable state Hermit has).
...
const DB_FILE_NAME: &str = "hermit.db";
...
fn write_backup(
    ...
    let file_name = format!("hermit-backup-{}.zip", now.format("%Y%m%d-%H%M%S"));
...
/// `program_data` (`data/hermit.db` → `{root}/hermit.db`, `config/…` →
...
mod tests {
    ...
    fn backup_then_restore_round_trips_db_and_config() {
        ...
        std::fs::write(program_data.join("hermit.db"), b"DBv1").unwrap();
        ...
        assert!(manifest.path.starts_with("hermit-backup-"));
        ...
        assert_eq!(
            std::fs::read(restore_root.join("hermit.db")).unwrap(),
            b"DBv1"
        );
        assert_eq!(
    ...
    async fn create_list_and_manifest_via_router() {
        ...
        assert!(created.path.starts_with("hermit-backup-"));
