...
//! from `ferrofin-model` rather than redefining them.

use std::net::{IpAddr, Ipv4Addr, Ipv6Addr};
use std::sync::LazyLock;

use fancy_regex::Regex;
use ferrofin_model::net::{AddressFamily, IpData, IpNetwork};
...
/// (`Jellyfin.Extensions.StringExtensions.LeftPart`, ported in `ferrofin-util`).
fn left_part(s: &str, needle: char) -> &str {
    ferrofin_util::string_extensions::left_part(s, needle)
}
