...
//!
//! Each test spins up an in-memory `hermit-db` database (migrations applied),
//! persists rows through [`HermitItemPersistenceService`], then queries them back
//! through [`HermitItemRepository`] / [`HermitItemCountService`] to exercise the
//! `InternalItemsQuery` → SQL translation end to end.

use std::sync::Arc;

use hermit_core::item_type_lookup::ItemTypeLookup;
use hermit_core::{HermitItemCountService, HermitItemPersistenceService, HermitItemRepository};
...
async fn save_then_retrieve_roundtrips() {
    ...
    let persist = HermitItemPersistenceService::new(db.clone());
...
async fn counts_group_by_kind() {
    let db = fresh_db().await;
    let persist = HermitItemPersistenceService::new(db.clone());
    let counts = HermitItemCountService::new(db.clone());
    persist
        .save_items(&[
