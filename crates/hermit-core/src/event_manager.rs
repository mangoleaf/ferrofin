//! [`HermitEventManager`] — the in-process domain-event publish seam.
//!
...
//!
//! The [`EventManager`](hermit_traits::events::EventManager) trait collapses the
...
//!
//! [`publish`]: HermitEventManager::publish
...
/// [`ServiceError`] lets [`HermitEventManager::publish`] log-and-continue,
...
pub struct HermitEventManager {
    consumers: Arc<RwLock<HashMap<String, Vec<EventConsumer>>>>,
}

impl std::fmt::Debug for HermitEventManager {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("HermitEventManager").finish_non_exhaustive()
    }
}

impl HermitEventManager {
...
#[async_trait]
impl EventManager for HermitEventManager {
...
mod tests {
    ...
    async fn publish_invokes_all_matching_consumers() {
        let manager = HermitEventManager::new();
