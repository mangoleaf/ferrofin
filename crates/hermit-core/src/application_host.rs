//! [`HermitServerApplicationHost`] — the concrete [`ServerApplicationHost`].
//!
...
pub struct HermitServerApplicationHost {
...
impl std::fmt::Debug for HermitServerApplicationHost {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("HermitServerApplicationHost")
            .field("network", &self.network)
            .finish_non_exhaustive()
    }
}

impl HermitServerApplicationHost {
...
#[async_trait]
impl ServerApplicationHost for HermitServerApplicationHost {
    ...
    fn expand_virtual_path(&self, path: &str) -> String {
        ...
        replace_ignore_ascii_case(
            &replace_ignore_ascii_case(
                path,
                HermitServerApplicationPaths::VIRTUAL_DATA_PATH,
                &data,
            ),
            HermitServerApplicationPaths::VIRTUAL_INTERNAL_METADATA_PATH,
            &metadata,
        )
    }
...
mod tests {
    ...
    async fn host(network: HostNetworkInfo) -> HermitServerApplicationHost {
        ...
        HermitServerApplicationHost::new(paths, cfg, network, "test-machine")
    }
