//! [`HermitMusicManager`] — the concrete [`MusicManager`].
//!
...
use hermit_traits::library::MusicManager;
...
#[derive(Clone)]
pub struct HermitMusicManager {
    items: Arc<dyn ItemRepository>,
}

impl std::fmt::Debug for HermitMusicManager {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("HermitMusicManager").finish_non_exhaustive()
    }
}

impl HermitMusicManager {
...
#[async_trait]
impl MusicManager for HermitMusicManager {
...
mod tests {
    ...
    fn manager(db: &Database) -> HermitMusicManager {
        let lookup: Arc<dyn hermit_traits::persistence::ItemTypeLookup> =
            Arc::new(ItemTypeLookup::new());
        HermitMusicManager::new(Arc::new(HermitItemRepository::new(db.clone(), lookup)))
    }
