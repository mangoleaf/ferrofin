//! [`HermitTrickplayManager`] — the [`TrickplayManager`] over the
//! `TrickplayInfos` table **plus** real tile generation.
//!
//! Port of `Jellyfin.Server.Implementations.Trickplay.TrickplayManager`. The
//! metadata rows ([`TrickplayInfoEntity`](hermit_db::entities::playback::TrickplayInfoEntity))
//! are stored/listed/deleted directly over `hermit-db` (there is no separate
...
//! - There is no per-library `LibraryOptions` in Hermit (no
...
use hermit_traits::options::ImageCollageOptions;
...
/// The per-width refresh context shared by [`HermitTrickplayManager`] helpers:
...
impl HermitTrickplayManager {
    ...
    async fn refresh_width(
        ...
        let frames_dir =
            std::env::temp_dir().join(format!("hermit_trickplay_{}", Uuid::new_v4().simple()));
        let result = self
    ...
    ///
    /// Port of `MoveGeneratedTrickplayDataAsync`. Hermit has no per-library
    /// `SaveTrickplayWithMedia` option (see `hermit-providers`'
...
mod tests {
    ...
    use async_trait::async_trait;
    use hermit_db::Database;
    use hermit_drawing::ImageCrateEncoder;
    use hermit_traits::configuration::ServerConfigurationManager;
    use hermit_traits::error::ServiceError;
    use hermit_traits::media_encoding::TrickplayFrameExtractor;
    use hermit_traits::system::PathManager;
    ...
    fn rig(db: &Database, options: TrickplayOptions, extractor: FakeExtractor) -> Rig {
        ...
        let pm: Arc<dyn PathManager> = Arc::new(HermitPathManager::new(paths));
        let extractor = Arc::new(extractor);
        let items = Arc::new(HermitItemRepository::new(
            db.clone(),
            Arc::new(ItemTypeLookup::new()),
        ));
        let mgr = HermitTrickplayManager::new(
            db.clone(),
            Arc::clone(&pm),
