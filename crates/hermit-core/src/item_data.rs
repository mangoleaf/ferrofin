...
//! ```
//!
//! Hermit's query paths keep using the `HermitLinkedChildren` /
...
//!
//! Writes are **read-modify-write** over the raw JSON object: keys Hermit does
...
/// the presence signal that Jellyfin (or a prior Hermit sync) owns this blob.
#[must_use]
...
/// into the cache; a row without one (created by Hermit before this module) is
...
mod tests {
    ...
    #[tokio::test]
    async fn reconcile_exports_pre_data_hermit_rows() {
        let db = test_db().await;
        let boxset = Uuid::new_v4();
        let child = Uuid::new_v4();
