//! [`HermitSystemManager`] — the concrete [`SystemManager`].
//!
...
//!   [`HermitServerApplicationPaths`](crate::app_paths::HermitServerApplicationPaths)
...
//!   (`StopApplication`, restart flag). Not a hermit-traits trait — the C#
...
use hermit_traits::system::{ServerApplicationHost, ServerApplicationPaths};
...
pub struct SystemHostFacts {
    ...
    pub completed_installations: Vec<hermit_model::updates::InstallationInfo>,
}
...
pub struct HermitSystemManager {
...
impl std::fmt::Debug for HermitSystemManager {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("HermitSystemManager")
            .field("facts", &self.facts)
            .finish_non_exhaustive()
    }
}

impl HermitSystemManager {
...
mod tests {
    ...
    use crate::application_host::{HermitServerApplicationHost, HostNetworkInfo};
    use crate::configuration_manager::HermitServerConfigurationManager;
    use hermit_traits::system::SystemManager as _;
    ...
    async fn build() -> (HermitSystemManager, Arc<FakeLifecycle>) {
        ...
        let host: Arc<dyn ServerApplicationHost> = Arc::new(HermitServerApplicationHost::new(
            Arc::clone(&paths),
        ...
        let facts = SystemHostFacts {
            ...
            product_name: Some("Hermit".to_owned()),
            system_id: Some("abc123".to_owned()),
            package_name: Some("hermit-docker".to_owned()),
            transcoding_temp_path: None,
            completed_installations: vec![],
        };
        let mgr = HermitSystemManager::new(
            host,
            Arc::clone(&cfg) as Arc<dyn ServerConfigurationManager>,
            paths,
    ...
    async fn public_info_is_the_unauthenticated_subset() {
        ...
        let info = mgr
            ...
            .unwrap();
        assert_eq!(info.version.as_deref(), Some("10.9.0"));
        assert_eq!(info.product_name.as_deref(), Some("Hermit"));
        assert_eq!(info.server_name.as_deref(), Some("test-machine"));
    }
