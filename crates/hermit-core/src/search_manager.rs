//! [`HermitSearchManager`] — the concrete [`SearchManager`].
//!
...
use hermit_traits::library::{SearchManager, SearchResult};
...
#[derive(Clone)]
pub struct HermitSearchManager {
    items: Arc<dyn ItemRepository>,
}

impl std::fmt::Debug for HermitSearchManager {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("HermitSearchManager")
            .finish_non_exhaustive()
    }
}

impl HermitSearchManager {
...
#[async_trait]
impl SearchManager for HermitSearchManager {
...
mod tests {
    ...
    fn manager(db: &Database) -> HermitSearchManager {
        let lookup: Arc<dyn hermit_traits::persistence::ItemTypeLookup> =
            Arc::new(ItemTypeLookup::new());
        HermitSearchManager::new(Arc::new(HermitItemRepository::new(db.clone(), lookup)))
    }
