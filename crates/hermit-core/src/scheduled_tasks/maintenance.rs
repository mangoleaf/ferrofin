...
/// (see `HermitItemPersistenceService`) whose `RetentionDate` passed the limit
/// are removed.
...
pub struct MoveTrickplayImagesTask {
    trickplay: Arc<crate::trickplay_manager::HermitTrickplayManager>,
}
...
impl MoveTrickplayImagesTask {
    ...
    #[must_use]
    pub fn new(trickplay: Arc<crate::trickplay_manager::HermitTrickplayManager>) -> Self {
        Self { trickplay }
    }
}
...
impl ScheduledTask for MoveTrickplayImagesTask {
    ...
    async fn execute(&self, progress: &TaskProgress) -> Result<(), ServiceError> {
        use hermit_traits::trickplay::TrickplayManager as _;
...
mod tests {
    ...
    use hermit_model::tasks::TaskTriggerInfoType;
    use hermit_traits::activity::{ActivityLogCreate, ActivityLogQuery, ActivityManager};
    use hermit_traits::configuration::ServerConfigurationManager;
    use hermit_traits::error::ServiceError;
    ...
    struct FakeConfig {
        ...
        paths: Arc<crate::HermitServerApplicationPaths>,
    }
    ...
    impl ActivityManager for FakeActivity {
        async fn get_paged_result(
            ...
            hermit_model::querying::QueryResult<hermit_model::activity::ActivityLogEntry>,
            ServiceError,
        > {
            unimplemented!("fake")
        }
    ...
    async fn clean_logs_respects_retention_and_serilog_exclusion() {
        ...
        let old = touch(&logs, "hermit-2026-01-01.log");
        backdate(&old, 4);
        let managed = touch(&logs, "log_managed.log");
        backdate(&managed, 40);
        let fresh = touch(&logs, "hermit-today.log");
    ...
    async fn collections_cleanup_removes_dead_children_only() {
        ...
        use crate::{HermitCollectionManager, HermitLinkedChildrenService, HermitPlaylistManager};
        use hermit_model::data::BaseItemKind;
        use uuid::Uuid;
        ...
        let collections: Arc<dyn CollectionManager> = Arc::new(HermitCollectionManager::new(
            db.clone(),
            Arc::clone(&library),
            Arc::clone(&linked),
        ));
        let playlists: Arc<dyn PlaylistManager> = Arc::new(HermitPlaylistManager::new(
            db.clone(),
            Arc::clone(&library),
            Arc::clone(&linked),
        ));
    ...
    async fn move_trickplay_task_pages_and_completes() {
        ...
        let trickplay = Arc::new(crate::trickplay_manager::HermitTrickplayManager::new(
            db.clone(),
            Arc::new(crate::HermitPathManager::new(app_paths)),
            config,
            Arc::new(crate::HermitItemRepository::new(
                db,
                Arc::new(crate::ItemTypeLookup::new()),
            )),
            Arc::new(NoExtract),
            Arc::new(hermit_drawing::ImageCrateEncoder::new()),
        ));
