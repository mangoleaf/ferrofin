...
//! `hermit-extensions`, next to the segment providers it runs.
//!
...
use hermit_model::entities_media::{MediaStream, VirtualFolderInfo};
use hermit_model::tasks::{TaskTriggerInfo, TaskTriggerInfoType};
use hermit_traits::chapters::ChapterManager;
use hermit_traits::error::ServiceError;
use hermit_traits::library::{LibraryManager, VirtualFolderManager};
use hermit_traits::media_encoding::MediaEncoder;
use hermit_traits::options::InternalItemsQuery;
use hermit_traits::persistence::{KeyframeRepository, MediaStreamQuery, MediaStreamRepository};
use hermit_traits::providers::{MetadataRefreshOptions, ProviderManager};
use hermit_traits::stubs::LyricManager;
use hermit_traits::subtitles::{SubtitleManager, SubtitleMediaType, SubtitleSearchRequest};
...
/// ffprobe extractor in `hermit-keyframes` (the upstream cache decorator's
        ...
        while start_index < total {
            ...
            for item in &items {
                ...
                let extracted = tokio::task::spawn_blocking(move || {
                    hermit_keyframes::ff_probe::get_keyframe_data(&probe, &path)
                })
                .await
...
/// `hermit-mediaencoding` uses for its `Transcoder`); [`TokioFfmpegRunner`] is
/// the real impl.
#[async_trait]
...
mod tests {
    ...
    use hermit_model::configuration::MediaPathInfo;
    use hermit_model::entities::CollectionTypeOptions;
    use hermit_model::entities::MediaStreamType as StreamTypeEnum;
    ...
    use hermit_model::providers::SubtitleProviderInfo;
    use hermit_traits::subtitles::SubtitleResponse;
  ...
  Integrated loudness:\n\
    ...
    impl TrickplayManager for FakeTrickplay {
        ...
        async fn get_trickplay_resolutions(
            ...
            std::collections::HashMap<i32, hermit_db::entities::playback::TrickplayInfoEntity>,
            ServiceError,
        > {
            unimplemented!("fake")
        }
    ...
    async fn audio_normalization_measures_albums_and_tracks() {
        ...
        let paths = Arc::new(crate::HermitServerApplicationPaths::new(
    ...
    async fn keyframe_task_skips_extracted_items_and_survives_probe_failure() {
        ...
        let keyframes: Arc<dyn KeyframeRepository> =
            Arc::new(crate::HermitKeyframeRepository::new(db.clone()));
    ...
    async fn chapter_task_fixture(
        ...
        let app_paths = Arc::new(crate::HermitServerApplicationPaths::new(
            media.path().join("data"),
            media.path().join("logs"),
            media.path().join("config"),
            media.path().join("cache"),
        ...
        let path_manager: Arc<dyn PathManager> =
            Arc::new(crate::HermitPathManager::new(Arc::clone(&app_paths)));
