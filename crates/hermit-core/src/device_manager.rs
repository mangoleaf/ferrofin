//! [`HermitDeviceManager`] — the concrete [`DeviceManager`] over `hermit-db`.
//!
...
#[derive(Clone)]
pub struct HermitDeviceManager {
    db: Database,
...
impl std::fmt::Debug for HermitDeviceManager {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("HermitDeviceManager")
            .finish_non_exhaustive()
    }
}

impl HermitDeviceManager {
...
#[async_trait]
impl DeviceManager for HermitDeviceManager {
...
mod tests {
    ...
    async fn create_then_query_and_token_lookup() {
        ...
        let mgr = HermitDeviceManager::new(db);
    ...
    async fn can_access_device_honors_admin_and_all_devices() {
        ...
        let mgr = HermitDeviceManager::new(db.clone());
