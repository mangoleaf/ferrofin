//! [`HermitQuickConnect`] — the concrete [`QuickConnect`] pairing flow.
//!
...
//! [`ServerConfigurationManager`](hermit_traits::configuration::ServerConfigurationManager)'s
//! `quick_connect_available` flag, and delegates the actual session creation to
//! the injected [`SessionManager`](hermit_traits::session::SessionManager)'s
...
//! `AuthenticationResult` envelope that has no `hermit-model` port yet).
//!
...
impl std::fmt::Debug for HermitQuickConnect {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("HermitQuickConnect").finish_non_exhaustive()
    }
}

impl HermitQuickConnect {
...
#[async_trait]
impl QuickConnect for HermitQuickConnect {
...
mod tests {
    ...
    fn manager(enabled: bool) -> HermitQuickConnect {
        HermitQuickConnect::new(
