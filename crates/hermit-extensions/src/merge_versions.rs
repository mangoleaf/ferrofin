...
//! - the four `/MergeVersions/*` routes stay in `hermit-api` (the thin HTTP
...
//! - Hermit models a version group solely by the `PrimaryVersionId` pointer;
...
//!   fire-and-forget lambdas (dropping errors); Hermit awaits sequentially.
...
/// `MergeVersionsManager` over Hermit's persistence seams.
...
fn episode_merge_key(ep: &BaseItemEntity, provider_id: impl Fn(&str) -> Option<String>) -> String {
    ...
    if let (Some(parent), Some(index)) = (ep.parent_index_number, ep.index_number) {
        // No `IndexNumberEnd` column exists in Hermit's schema; the trailing
...
mod tests {
    ...
    use hermit_core::{
        ...
        HermitLibraryManager, HermitPeopleRepository,
    };
    use hermit_db::Database;
    use hermit_model::updates::{PackageInfo, RepositoryInfo};
    use hermit_traits::plugins::PluginImage;
    ...
    impl VirtualFolderManager for FakeVirtualFolders {
        ...
        async fn add_virtual_folder(
            &self,
            _name: &str,
            _collection_type: Option<hermit_model::entities::CollectionTypeOptions>,
            _options: &hermit_model::configuration::LibraryOptions,
        ) -> Result<(), ServiceError> {
            unimplemented!()
        }
    ...
    fn service_over(
        ...
        let items = Arc::new(HermitItemRepository::new(db.clone(), lookup));
    ...
    fn stored_type(kind: BaseItemKind) -> &'static str {
        hermit_core::item_type_lookup::stored_type_name(kind).expect("stored type name")
    }
    ...
    async fn set_episode_fields(
        ...
        let lookup: Arc<dyn hermit_traits::persistence::ItemTypeLookup> =
            Arc::new(hermit_core::item_type_lookup::ItemTypeLookup::new());
        let mut item = HermitItemRepository::new(db.clone(), lookup)
            .retrieve_item(id)
            .await
            .expect("read")
            .expect("row");
    ...
    async fn tasks_register_the_upstream_pair_and_gate_on_enabled() {
        ...
        let cx = ExtensionContext {
            library: Arc::new(HermitLibraryManager::new(
                Arc::new(HermitItemRepository::new(
                    db.clone(),
                    Arc::new(hermit_core::item_type_lookup::ItemTypeLookup::new()),
                )),
                Arc::new(HermitItemCountService::new(db.clone())),
