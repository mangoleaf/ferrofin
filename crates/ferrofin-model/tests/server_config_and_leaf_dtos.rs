...
#[test]
fn enum_wire_values() {
    use ferrofin_model::configuration::{ProcessPriorityClass, SubtitlePlaybackMode};
    use ferrofin_model::live_tv::{ChannelType, KeepUntil, ProgramAudio};
    use ferrofin_model::plugins::PluginStatus;
    use ferrofin_model::sync_play::{GroupRepeatMode, GroupUpdateType};
    use ferrofin_model::tasks::TaskCompletionStatus;
...
fn iso_field_casing_globalization() {
    use ferrofin_model::globalization::CountryInfo;
...
fn installation_info_id_is_named_guid() {
    use ferrofin_model::updates::InstallationInfo;
...
fn package_info_and_version_info_use_camel_case() {
    use ferrofin_model::updates::{PackageInfo, VersionInfo};
...
fn group_update_is_internally_tagged_on_type() {
    use ferrofin_model::sync_play::{GroupUpdate, UserJoinedUpdate};
...
fn activity_log_severity_uses_log_level_names() {
    use ferrofin_model::activity::LogLevel;
