...
pub fn intel_vid_filter_chain(input: &ChainInput<'_>) -> FilterChain {
    ...
    // The D3D11 pipeline is phase 5b. Until it lands a Windows QSV job takes
    // the software chain, which is where upstream sends it whenever OpenCL or
    // d3d11va is missing — narrower, and reachable only on Windows, where QSV
    // is gated out of the hardware path entirely for now.
    sw_vid_filter_chain(input)
}
...
pub fn intel_qsv_vaapi_vid_filters_prefered(input: &ChainInput<'_>) -> FilterChain {
    ...
    if is_qsv_in_qsv_out {
        if input.subtitle.is_some() {
            // As with `overlay_vaapi`, the overlay rescales, so the plane is
            // generated smaller to move less across the bus.
