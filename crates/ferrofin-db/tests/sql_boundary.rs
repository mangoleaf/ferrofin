...
//! the job: `ferrofin-db` itself, `*_repository.rs` modules, `translate_query.rs`,
...
fn is_exempt(rel: &str) -> bool {
    rel.starts_with("crates/ferrofin-db/src/")
...
const CEILINGS: &[(&str, usize)] = &[
    ...
    ("crates/ferrofin-core/src/device_manager.rs", 13),
    (
    ...
    ("crates/ferrofin-core/src/music_manager.rs", 1),
    ("crates/ferrofin-core/src/next_up_service.rs", 5),
    ("crates/ferrofin-core/src/playback_metrics.rs", 4),
    ("crates/ferrofin-core/src/scheduled_tasks/library.rs", 17),
    ...
    ("crates/ferrofin-core/src/test_support.rs", 10),
...
fn sql_stays_behind_the_repository_boundary() {
    ...
    assert!(
        ...
        "SQL boundary ratchet violated:\n  {}\n\n\
         Rule: new SQL goes in a repository/persistence module (ferrofin-db, \
         *_repository.rs, translate_query.rs, item_persistence_service.rs, \
         ...
         Ceilings in crates/ferrofin-db/tests/sql_boundary.rs only ratchet down: \
